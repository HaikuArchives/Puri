...
ICSMainWindow::ICSMainWindow(ICSInfo info)
	: BWindow(BRect(), "Chess Server", B_TITLED_WINDOW_LOOK, B_NORMAL_WINDOW_FEEL,
...
ICSMainWindow::_ParseBlock(const char*& p1)
{
	...
	if (ident == 2000) {
		...
		switch (code) {
			...
			default:
				!out << "\n\n\n*******!!!!! BLOCK-Code : " << code << ": Unhandled" << std::endl;
				return true;
		}
	}
...
ICSMainWindow::_SaveSettings(void)
{
	...
	settings << "Split10" << (float)fSplitView1->ItemWeight((int32)0) << "Split11"
			 << (float)fSplitView1->ItemWeight((int32)1) << "Split20"
			 << (float)fSplitView2->ItemWeight((int32)0) << "Split21"
			 << (float)fSplitView2->ItemWeight((int32)1) << "Split30"
			 << (float)fSplitView3->ItemWeight((int32)0) << "Split31"
			 << (float)fSplitView3->ItemWeight((int32)1) << "Split40"
			 << (float)fSplitView4->ItemWeight((int32)0) << "Split41"
			 << (float)fSplitView4->ItemWeight((int32)1) << std::endl;
}
