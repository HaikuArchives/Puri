...
MovesListView::MovesListView(int32 replyCode)
	: BView("moves_list_view", B_WILL_DRAW | B_FULL_UPDATE_ON_RESIZE | B_FRAME_EVENTS),
...
MovesListView::Draw(BRect updateRect)
{
	...
	for (uint i = 0; i < fWhiteMoves.size(); ++i) {
		...
		if (move.first == NULL) {
			...
			DrawBitmapAsync(
				move.first, fIR.OffsetToSelf(offsetX, tempY - fTotalFH + fRowSpace).LeftTop());
		...
		StrokeLine(BPoint(fMargin.left, tempY), BPoint(frameWidth - fMargin.right, tempY));
	}
...
{
	uint32 visibleRows = (Frame().Height() - fMargin.top - fMargin.bottom) / fTotalFH;
...
{
	int32 maxRange;
	maxRange = fMargin.top + fMargin.bottom + fWhiteMoves.size() * fTotalFH - Frame().Height();
...
{
	return (Frame().Height() / (fMargin.top + fMargin.bottom + fWhiteMoves.size() * fTotalFH));
}
...
ScrollView::ScrollView(const char* name, BView* target, bool vertical, bool horizontal,
	uint32 flags, uint32 resizingMode, border_style border)
...
ScrollView::MessageReceived(BMessage* message)
{
	switch (message->what) {
		case M_REPLY:
		{
			...
			ScrollBar(B_VERTICAL)->SetSteps(fTarget->_ScrollStep(), fTarget->_ScrollStep());
			break;
		}
