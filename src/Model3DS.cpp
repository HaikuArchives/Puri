	  ...
	  fID(0)
{
	...
	if (showTexture) {
		fTextureIsOn = true;
		fTexture
			= loadPNGTexture(Tools::AppPath().Append("/data/Texture/Board/board.png").String());
	}
}
	...
	for (unsigned long i = 0; i < (*fModel->meshes)->nfaces; i++) {
		if (fHighlight) {
			...
			glMaterialfv(GL_FRONT, GL_DIFFUSE,
				fModel->materials[(*fModel->meshes)->faces[i].material]->diffuse);
			glMaterialfv(GL_FRONT, GL_SPECULAR,
				fModel->materials[(*fModel->meshes)->faces[i].material]->specular);
		} else {
			// set material
			// glMaterialf (GL_FRONT, GL_SHININESS, fModel->materials[
			// (*fModel->meshes)->faces[i].material ]->shininess);
			glMaterialf(GL_FRONT, GL_SHININESS, 128);
			glMaterialfv(GL_FRONT, GL_AMBIENT,
				fModel->materials[(*fModel->meshes)->faces[i].material]->ambient);
			glMaterialfv(GL_FRONT, GL_DIFFUSE,
				fModel->materials[(*fModel->meshes)->faces[i].material]->diffuse);
			glMaterialfv(GL_FRONT, GL_SPECULAR,
				fModel->materials[(*fModel->meshes)->faces[i].material]->specular);
		...
		glVertex3fv((*fModel->meshes)->vertices[(*fModel->meshes)->faces[i].index[0]]);
		glVertex3fv((*fModel->meshes)->vertices[(*fModel->meshes)->faces[i].index[1]]);
		glVertex3fv((*fModel->meshes)->vertices[(*fModel->meshes)->faces[i].index[2]]);
	}
...
void
Model3DS::SetDeltaPos(float const& kDeltaX, float const& kDeltaY)
{
	fPosX += kDeltaX;
	fPosY += kDeltaY;
...
GLuint
Model3DS::_LoadTexture(BString const& filename, int& width, int& height)
{
	...
	: fPosX( 0 ), fPosY( 0 ), fPosZ( 0 )
{
	...
	glBindBuffer(GL_ARRAY_BUFFER, fVertexVBO);
	glBufferData(GL_ARRAY_BUFFER, 3 * sizeof(float) * 3 * fTotalFaces, orig_vertices,
GL_STATIC_DRAW);
