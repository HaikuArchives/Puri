...
SidebarView::SidebarView(void)
	: BGroupView("sidebar_view", B_HORIZONTAL, 0),
	  fAutohiding(false)
{
	ImageButton* movenowButton = new ImageButton("move_now", new BMessage(MENU_GAME_MOVENOW), 0.3);
	movenowButton->SetToolTip("Move Now");
	ImageButton* newgameButton = new ImageButton("new", new BMessage(MENU_GAME_NEWGAME), 0.3);
	newgameButton->SetToolTip("New Game");
	ImageButton* flipboardButton
		= new ImageButton("refresh", new BMessage(MENU_VIEW_FLIPBOARD), 0.3);
	flipboardButton->SetToolTip("Flip Board");
	ImageButton* fullscreenButton
		= new ImageButton("fullscreen", new BMessage(MENU_VIEW_FULLSCREEN), 0.3);
	fullscreenButton->SetToolTip("Fullscreen");
	ImageButton* view3DButton = new ImageButton("view_3d", new BMessage(MENU_VIEW_3D), 0.3);
	view3DButton->SetToolTip("3D View");
	ImageButton* hidemenuButton
		= new ImageButton("hide_menu", new BMessage(MENU_VIEW_SHOWMENU), 0.3);
	hidemenuButton->SetToolTip("Hide Menu");
	ImageButton* quitButton = new ImageButton("exit", new BMessage(MENU_APP_QUIT), 0.3);
