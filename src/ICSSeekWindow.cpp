...
ICSSeekWindow::ICSSeekWindow(BWindow* target)
	: BWindow(BRect(), "Seek", B_TITLED_WINDOW_LOOK, B_FLOATING_ALL_WINDOW_FEEL,
		  B_ASYNCHRONOUS_CONTROLS | B_AUTO_UPDATE_SIZE_LIMITS | B_OUTLINE_RESIZE
...
ICSSeekWindow::_SaveSettings(void)
{
	...
	settings << "TimeMin" << fTimeMin->Text() << "TimeMax" << fTimeMax->Text() << "IncMin"
			 << fIncMin->Text() << "IncMax" << fIncMax->Text() << "Rated" << fRated->Value()
			 << "WithClock" << fWithClock->Value() << "Automatic" << fColorAutomatic->Value()
			 << "White" << fColorWhite->Value() << "Black" << fColorBlack->Value() << std::endl;
  ...
  */
	command << BLK_SEEK << " seek " << atoi(fTimeMin->Text()) << " " << atoi(fIncMin->Text());
...
ICSSeekWindow::_LoadSettings(void)
{
	...
	BString str;
	str = "5";
	settings << "TimeMin" >> str;
	fTimeMin->SetText(str);
	str = "5";
	settings << "TimeMax" >> str;
	fTimeMax->SetText(str);
	str = "3";
	settings << "IncMin" >> str;
	fIncMin->SetText(str);
	str = "3";
	...
	int32 value;
	value = 1;
	settings << "WithClock" >> value;
	fWithClock->SetValue(value);
	value = 1;
	settings << "Automatic" >> value;
	fColorAutomatic->SetValue(value);
	value = 1;
	settings << "White" >> value;
	fColorWhite->SetValue(value);
	value = 1;
	settings << "Black" >> value;
	fColorBlack->SetValue(value);
	value = 1;
	settings << "Rated" >> value;
	fRated->SetValue(value);
