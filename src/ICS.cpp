...
ICS::ICS(ICSInfo const& info, uint32 replyCode, BLooper* targetLooper, BHandler* targetHandler,
	int32 priority)
		...
		if (n > 0) {
			if (isLoggedIn == false) {
				if (strstr(buf, "login:") != NULL)
					data.target_looper->PostMessage(ICS_ASKS_USERNAME, data.target_handler);
				else if (strstr(buf, "password:") != NULL)
					data.target_looper->PostMessage(ICS_ASKS_PASSWORD, data.target_handler);
				else if (strstr(buf, "**** Starting") != NULL) {
					isLoggedIn = true;
					data.target_looper->PostMessage(ICS_LOGGED_IN, data.target_handler);
				} else if (strstr(buf, "**** Invalid password") != NULL)
					data.target_looper->PostMessage(ICS_DISCONNECTED, data.target_handler);
