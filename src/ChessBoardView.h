...
public:
	...
	void CancelSelection(void) { _CancelSelection(); }

	void ShowValidMoves(bool showValidMoves) { fShowValidMoves = showValidMoves; }


	bool IsWhiteTurn(void) { return fBoard->IsWhiteTurn(); }

	BString LastMoveICSF(void) { return fBoard->LastMoveICSF(); }

	BString MovesEF(void) { return fBoard->MovesEF(); }


	void SetUserCanMove(bool userCanMove = true) { fUserCanMove = userCanMove; }


	BString GetMaterial(void) { return fBoard->GetMaterial(); }

	BackBoard* Board(void) { return fBoard; }
...
private:
	...
	BBitmap* _ImageBoard(Tuple<uint> const& kTuple) { return _ImageBoard(kTuple.x, kTuple.y); }
	...
	void _MakeArrowHead(
		BPoint& p1, BPoint& p2, BPoint& p3, float a, BPoint const& shift = BPoint(0, 0));
	...
	std::vector<BPoint> _CreateArrow2D(Tuple<int> const& from, Tuple<int> const& to);

	Model3DS*& _3DModel(uint const& x, uint const& y);


	inline Model3DS*& _Board(Tuple<uint> const& kPos) { return fvPieceModel[kPos.x][kPos.y]; }
	...
	inline const int& _BBoard(Tuple<uint> kCoord) const { return fBoard->_Board(kCoord + 2); }
