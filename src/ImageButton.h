...
public:
	ImageButton(BString imageName, BMessage* message = nullptr, float marginProportion = 0,
		int frameBehaviour = 0, const char* name = "im_button",
		uint32 flags = B_FRAME_EVENTS | B_WILL_DRAW | B_NAVIGABLE | B_FULL_UPDATE_ON_RESIZE
					   | B_DRAW_ON_CHILDREN);
...
private:
	BBitmap* _RescaleBitmap(const BBitmap* src, int32 width, int32 height);
