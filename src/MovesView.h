...
private:
	std::pair<BBitmap*, const char*> _SplitMove(BString moveStr, bool isWhite = true);
...
public:
	ScrollView(const char* name, BView* target, bool vertical = true, bool horizontal = false,
		uint32 flags = B_SUPPORTS_LAYOUT | B_FULL_UPDATE_ON_RESIZE,
		uint32 resizingMode = B_FOLLOW_ALL_SIDES, border_style border = B_FANCY_BORDER);
...
public:
	...
	void AddMove(BString const& str, bool addWhite = true) { fMovesList->AddMove(str, addWhite); }
	...
	void GotoMove(uint idx) { fMovesList->GotoMove(idx); }
