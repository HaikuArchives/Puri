...
BasicMaterialView::Draw(BRect updateRect)
{
	...
	if (fMergedMode == false) {
		...
		for (uint i = 0; i < fWPieces.size(); ++i)
			DrawBitmapAsync(fBM[fWPieces[i]], fIR.OffsetBySelf(fIR.Width(), 0).LeftTop());

		fIR.OffsetTo(-fIR.Width(), Frame().bottom - fIR.Height());

		for (uint i = 0; i < fBPieces.size(); ++i)
			DrawBitmapAsync(fBM[fBPieces[i]], fIR.OffsetBySelf(fIR.Width(), 0).LeftTop());
	} else {
		int counter = 0;
...
BasicMaterialView::FrameResized(float newWidth, float newHeight)
{
	...
	if (max < 3 || abs((long int)(fWPieces.size() - fBPieces.size())) < 2
...
BasicMaterialView::SetMaterial(BString const& str)
{
	...
	for (int i = 0; i < str.Length(); ++i) {
		switch (str[i]) {
			case 'K':
				fWPieces.push_back(wK);
				break;
			case 'Q':
				fWPieces.push_back(wQ);
				break;
			case 'R':
				fWPieces.push_back(wR);
				break;
			case 'B':
				fWPieces.push_back(wB);
				break;
			case 'N':
				fWPieces.push_back(wN);
				break;
			case 'P':
				fWPieces.push_back(wP);
				break;

			case 'k':
				fBPieces.push_back(bK);
				break;
			case 'q':
				fBPieces.push_back(bQ);
				break;
			case 'r':
				fBPieces.push_back(bR);
				break;
			case 'b':
				fBPieces.push_back(bB);
				break;
			case 'n':
				fBPieces.push_back(bN);
				break;
			case 'p':
				fBPieces.push_back(bP);
				break;
