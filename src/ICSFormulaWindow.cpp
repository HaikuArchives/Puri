...
ICSFormulaWindow::ICSFormulaWindow(BWindow* target)
	: BWindow(BRect(), "Formula", B_TITLED_WINDOW_LOOK, B_FLOATING_ALL_WINDOW_FEEL,
	  ...
	  fTarget(target)
{
	...
	settings << "EloMin" >> e1 << "EloMax" >> e2 << "TimeMin" >> t1 << "TimeMax" >> t2 << "IncMin"
		>> i1 << "IncMax" >> i2;
	...
	settings << "rated" >> rated << "unrated" >> unrated << "activated" >> activated;
...
ICSFormulaWindow::_Apply(void)
{
	...
	Settings settings("FormulaWnd");
	settings << "EloMin" << fEloMin->Text() << "EloMax" << fEloMax->Text() << "TimeMin"
			 << fTimeMin->Text() << "TimeMax" << fTimeMax->Text() << "IncMin" << fIncMin->Text()
			 << "IncMax" << fIncMax->Text() << "rated" << fRated->Value() << "unrated"
			 << fUnrated->Value() << "activated" << fActivate->Value() << std::endl;
}
