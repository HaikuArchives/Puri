...
IntroWindow::IntroWindow(void)
	: BWindow(BRect(), "Please Choose", B_NO_BORDER_WINDOW_LOOK, B_FLOATING_ALL_WINDOW_FEEL,
...
void
IntroWindow::MessageReceived(BMessage* message)
{
	switch (message->what) {
		case M_ONLINE:
		{
			if (BString(fServerTC->Text()).IsEmpty() | BString(fServerTC->Text()).IsEmpty()
				| BString(fUsernameTC->Text()).IsEmpty() | BString(fPasswordTC->Text()).IsEmpty())
				return;
			ICSInfo info;
			...
			Settings settings("IntroWnd");
			settings << "user" << info.user << "pass" << info.pass << "remember"
					 << fRemember->Value() << std::endl;
			Close();
			break;
		}
