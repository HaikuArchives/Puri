...
public:
	...
	BString const& LastMoveString(void) { return fLastMoveStr; }

	BString LastMoveICSF(void) { return fPlayedMoves.back().ICSF(); }
...
private:
	...
	inline bool _IsSlider(int const& kPiece) { return ((kPiece & Queen) != 0); }

	inline bool _IsFlag(int const& kPiece, int const& kFlag) { return ((kPiece & kFlag) == kFlag); }


	inline bool _FindMove(Tuple<uint> const& from, Tuple<uint> const& to) const
	{
		return (find(fMoves.begin(), fMoves.end(), Move(from, to)) != fMoves.end());
	}


	inline bool _FindMove(Move const& kMove, std::list<Move> const& kMoveList) const
	{
		return (find(kMoveList.begin(), kMoveList.end(), kMove) != kMoveList.end());
	}


	inline bool _FindMove(
		Tuple<uint> const& from, Tuple<uint> const& to, std::list<Move> const& kMoveList) const
	{
		return (_FindMove(Move(from, to), kMoveList));
	}


	inline int const _GetColor(int const& kPiece) { return (0); }
	...
	inline int& _Board(Tuple<uint> const& kPosition) { return fvBoard[kPosition.x][kPosition.y]; }


	inline int& _Board(uint const& x, uint const& y) { return fvBoard[x][y]; }


	inline void _Turn(void) { fTurn = !fTurn; }
