...
BlinkerView::BlinkerView(void)
	: BView(BRect(), "blinker_view", B_FOLLOW_ALL, B_WILL_DRAW | B_FULL_UPDATE_ON_RESIZE),
	  fIsOn(false)
{
...
BlinkerView::Draw(BRect updateRect)
{
	...
	if (fIsOn)
		FillEllipse(BPoint(radius, radius), radius * 0.7, radius * 0.7, fGradientOn);
	else
		FillEllipse(BPoint(radius, radius), radius * 0.7, radius * 0.7, fGradientOff);
}
